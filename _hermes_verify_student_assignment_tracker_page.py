"""Acceptance smoke checks for the Student Assignment Tracker page migration.

Run from the repo root with the venv active::

    python _hermes_verify_student_assignment_tracker_page.py

Verifies that ``/student-assignment-tracker.html`` and the extensionless
``/student-assignment-tracker`` alias both render through FastAPI/Jinja with
HTTP 200, that the two routes render identical HTML, that key page copy and
section anchors are present, that the shared header/footer partials are
rendered (not duplicated page-local markup), that static asset URLs point at
``/static/``, and that exactly one ``<main id="main">`` survives.
"""
...
ROUTES = ["/student-assignment-tracker.html", "/student-assignment-tracker"]
...
def main() -> int:
    ...
    # The two routes must render identical HTML (shared render helper + context).
